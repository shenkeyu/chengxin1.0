...
response.setContentType("text/html;charset=UTF-8");
     ...
     if(request.getParameter("gerenIDcardxiang")!=null)
     {
   // s1=new String(request.getParameter("gerenIDcardxiang").getBytes("ISO-8859-1"),"gb2312");
    //s1=s1.trim();
    s1=request.getParameter("gerenIDcardxiang").trim();
     session.setAttribute("gerenIDcardxiang", s1);
     };
	String panduan=session.getAttribute("gerenIDcardxiang").toString().trim();//判断卡号不为空再执行
	if(panduan!=""){
	%>
	<%
     //连接数据库并初始数据
      try{
       ...
       String sql="select * from persondorecord ";
		Calendar c = Calendar.getInstance();
       String currentyear=Integer.toString(c.get(Calendar.YEAR));
       String currentmonth=Integer.toString(c.get(Calendar.MONTH)+1);
       if(session.getAttribute("gerenIDcardxiang")==null||session.getAttribute("gerenIDcardxiang").equals(""))
       {
	      System.out.println("查询详单来源内容为空");
    	}else{
    	 System.out.println("s1:"+session.getAttribute("gerenIDcardxiang"));
	       sql=sql+" where personIDcard="+session.getAttribute("gerenIDcardxiang")+" and persondoyear="+currentyear+" and persondomonth="+currentmonth;
    	};
...
<center>
	...
	<tr>
	<td style="border:1px solid #CCCCCC;height:20px;"  align="center">用户ID</td>
	<td style="border:1px solid #CCCCCC;height:20px;"  align="center">用户名称</td>
	...
	<td style="border:1px solid #CCCCCC;height:20px;"  align="center"><%=rs.getString("persondo")%></td>
	<td style="border:1px solid #CCCCCC;height:20px;"  align="center"><%=rs.getString("personfen")%></td>
	</tr> 
     <%
      ...
      %>
       <td width=150>
        <a href="gerenxinyong/gerenxiangdan.jsp?ToPage=<%=1 %>">第一页</a>
       </td> 
       <td width=150>
        <a href="gerenxinyong/gerenxiangdan.jsp?ToPage=<%=ShowPage-1 %>">上一页</a>
       </td>
      <% 
       }
       ...
       %>
        <td width=150>
         <a href="gerenxinyong/gerenxiangdan.jsp?ToPage=<%=ShowPage+1 %>">下一页</a>
        </td>
        <td width=150>
         <a href="gerenxinyong/gerenxiangdan.jsp?ToPage=<%=PageCount %>">最后一页</a>
        </td>
        <%
         }
         %>
         ...
         <td colspan="4" align="center">
          <form action="gerenxinyong/gerenxiangdan.jsp" method="post" name="form1">
...
<%};//panduan的结束
 %>
<% /////////////////////////////////////////////分隔线//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////%>				
<%String panduanpp=session.getAttribute("gerenIDcardxiang").toString();//判断卡号不为空再执行
if(panduanpp==""){%>
