...
%>
<body>
<center><div style="font-size:20px;margin-top:20px">外分包商清单</div></center>
<%
...
response.setContentType("text/html;charset=UTF-8");
    ...
    //String ss=request.getParameter("mingzi").trim();
     session.setAttribute("chaxunwai", ss);
	};
 try{
	Class.forName("com.microsoft.sqlserver.jdbc.SQLServerDriver");//加载JDBC驱动器类
		...
		System.out.println(session.getAttribute("chaxunwai").toString());
		stmt3= con3.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
		String sql3="select * from wai where name like '%"+session.getAttribute("chaxunwai").toString().trim()+"%'";    
...
%>
								<td style="vertical-align:middle;align:left;width:323px">
										...
										<form style="center" action="waixiangdan.jsp" method="post" >
										<input name="wainamexiang" type="hidden" value="<%=rs3.getString("name").trim()%>">
