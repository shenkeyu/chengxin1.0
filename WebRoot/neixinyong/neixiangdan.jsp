...
response.setContentType("text/html;charset=UTF-8");
     ...
     if(request.getParameter("neinamexiang")!=null)
     {
   // s1=new String(request.getParameter("neinamexiang").getBytes("ISO-8859-1"),"gb2312");
    //s1=s1.trim();
    s1=request.getParameter("neinamexiang").trim();
     session.setAttribute("neinamexiang", s1);
     };
	String panduan=session.getAttribute("neinamexiang").toString().trim();//判断卡号不为空再执行
	if(panduan!=""){
	%>
	<%
     //连接数据库并初始数据
      try{
       ...
       String sql="select * from neidorecord ";
		Calendar c = Calendar.getInstance();
       String currentyear=Integer.toString(c.get(Calendar.YEAR));
       String currentmonth=Integer.toString(c.get(Calendar.MONTH)+1);
       if(session.getAttribute("neinamexiang")==null||session.getAttribute("neinamexiang").equals(""))
       {
	      System.out.println("查询详单来源内容为空");
    	}else{
    	 System.out.println("s1:"+session.getAttribute("neinamexiang"));
	       sql=sql+" where neiname like '"+session.getAttribute("neinamexiang")+"%' and neidoyear="+currentyear+" and neidomonth="+currentmonth;
    	};
...
<center>
    ...
    %>  
	<tr>
	<td style="border:1px solid #CCCCCC;height:20px;"  align="center"><%=rs.getString("neiname")%></td>
	<td style="border:1px solid #CCCCCC;height:20px;"  align="center"><%=rs.getString("neido")%></td>
	<td style="border:1px solid #CCCCCC;height:20px;"  align="center"><%=rs.getString("neifen")%></td>
	</tr> 
     <%
      ...
      %>
       <td width=150>
        <a href="neixinyong/neixiangdan.jsp?ToPage=<%=1 %>">第一页</a>
       </td> 
       <td width=150>
        <a href="neixinyong/neixiangdan.jsp?ToPage=<%=ShowPage-1 %>">上一页</a>
       </td>
      <% 
       }
       ...
       %>
        <td width=150>
         <a href="neixinyong/neixiangdan.jsp?ToPage=<%=ShowPage+1 %>">下一页</a>
        </td>
        <td width=150>
         <a href="neixinyong/neixiangdan.jsp?ToPage=<%=PageCount %>">最后一页</a>
        </td>
        <%
         }
         %>
         ...
         <td colspan="4" align="center">
          <form action="neixinyong/neixiangdan.jsp" method="post" name="form1">
...
<%};//panduan的结束
 %>
<% /////////////////////////////////////////////分隔线//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////%>				
<%String panduanpp=session.getAttribute("neinamexiang").toString();//判断卡号不为空再执行
if(panduanpp==""){%>
