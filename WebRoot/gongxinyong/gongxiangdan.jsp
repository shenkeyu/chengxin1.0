...
response.setContentType("text/html;charset=UTF-8");
     ...
     if(request.getParameter("gongnamexiang")!=null)
     {
   // s1=new String(request.getParameter("gongnamexiang").getBytes("ISO-8859-1"),"gb2312");
    //s1=s1.trim();
    s1=request.getParameter("gongnamexiang").trim();
     session.setAttribute("gongnamexiang", s1);
     };
	String panduan=session.getAttribute("gongnamexiang").toString().trim();//判断卡号不为空再执行
	if(panduan!=""){
	%>
	<%
     //连接数据库并初始数据
      try{
       ...
       String sql="select * from gongdorecord ";
		Calendar c = Calendar.getInstance();
       String currentyear=Integer.toString(c.get(Calendar.YEAR));
       String currentmonth=Integer.toString(c.get(Calendar.MONTH)+1);
       if(session.getAttribute("gongnamexiang")==null||session.getAttribute("gongnamexiang").equals(""))
       {
	      System.out.println("查询详单来源内容为空");
    	}else{
    	 System.out.println("s1:"+session.getAttribute("gongnamexiang"));
	       sql=sql+" where gongname like '"+session.getAttribute("gongnamexiang")+"%' and gongdoyear="+currentyear+" and gongdomonth="+currentmonth;
    	};
...
<center>
    ...
    %>  
	<tr>
	<td style="border:1px solid #CCCCCC;height:20px;"  align="center"><%=rs.getString("gongname")%></td>
	<td style="border:1px solid #CCCCCC;height:20px;"  align="center"><%=rs.getString("gongdo")%></td>
	<td style="border:1px solid #CCCCCC;height:20px;"  align="center"><%=rs.getString("gongfen")%></td>
	</tr> 
     <%
      ...
      %>
       <td width=150>
        <a href="gongxinyong/gongxiangdan.jsp?ToPage=<%=1 %>">第一页</a>
       </td> 
       <td width=150>
        <a href="gongxinyong/gongxiangdan.jsp?ToPage=<%=ShowPage-1 %>">上一页</a>
       </td>
      <% 
       }
       ...
       %>
        <td width=150>
         <a href="gongxinyong/gongxiangdan.jsp?ToPage=<%=ShowPage+1 %>">下一页</a>
        </td>
        <td width=150>
         <a href="gongxinyong/gongxiangdan.jsp?ToPage=<%=PageCount %>">最后一页</a>
        </td>
        <%
         }
         %>
         ...
         <td colspan="4" align="center">
          <form action="gongxinyong/gongxiangdan.jsp" method="post" name="form1">
...
<%};//panduan的结束
 %>
<% /////////////////////////////////////////////分隔线//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////%>				
<%String panduanpp=session.getAttribute("gongnamexiang").toString();//判断卡号不为空再执行
if(panduanpp==""){%>
