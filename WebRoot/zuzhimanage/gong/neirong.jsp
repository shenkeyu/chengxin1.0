     ...
     //连接数据库并初始数据
      try{
       ...
       String sql="select * from gong";
	       if(session.getAttribute("gongming").toString()!=null&&session.getAttribute("gongming").toString()!="")
	       {
	       sql=sql+" where name Like '%"+session.getAttribute("gongming").toString().trim()+"%'";
	    	};
