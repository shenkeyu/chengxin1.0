...
<%
	request.setCharacterEncoding("utf-8");
     ...
     session.setAttribute("waiming", request.getParameter("zuzhiming"));
System.out.println(session.getAttribute("waiming").toString());
     //连接数据库并初始数据
      try{
       ...
       String sql="select * from wai";
	       if(session.getAttribute("waiming").toString()!=null&&session.getAttribute("waiming").toString()!="")
	       {
	       sql=sql+" where name Like '%"+session.getAttribute("waiming").toString().trim()+"%'";
	    	};
